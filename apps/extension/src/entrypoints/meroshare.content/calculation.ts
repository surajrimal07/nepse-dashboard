...
function determinePersonality(
	...
	if (buyCount > 50 && churnRate > 0.5) {
		type = "Active Trader";
		description =
			"You trade frequently and aren't afraid to take profits or cut losses.";
	} else if (buySellRatio > 5 && churnRate < 0.2) {
		type = "Diamond Hands";
		description =
			"You accumulate stocks and rarely sell. A true long-term believer.";
	} else if (buyCount < 10) {
		type = "The Rookie";
		...
		type = "Swing Trader";
		description =
			"You seek balance, buying and selling regularly to capture short-term moves.";
	}
...
export async function generateAdvancedInsights(): Promise<AdvancedUserInsights> {
	...
	transactions.forEach((txn) => {
		...
		entry.turnoverRatio =
			entry.totalBoughtQty > 0 ? entry.totalSoldQty / entry.totalBoughtQty : 0;
		entry.activityDurationDays =
			(entry.lastTransactionDate! - entry.firstTransactionDate!) /
			(1000 * 60 * 60 * 24);
	});

	const txnValues = Array.from(txnMap.values());
	const mostActive = [...txnValues].sort(
		(a, b) => b.buyCount + b.sellCount - (a.buyCount + a.sellCount),
	)[0];
	const mostVolatile = [...txnValues].sort(
	...
	holdings.forEach((h) => {
		h.contributionPercent =
			totalVal > 0 ? (h.currentValue / totalVal) * 100 : 0;
		h.weightInPortfolio = h.contributionPercent;
	});

	// --- 3. Compute Portfolio Metrics (ACCURATE) ---
	const winLossRatio =
		losersCount > 0 ? winnersCount / losersCount : winnersCount;
	const profitFactor =
		totalGrossLoss > 0
	...
	const overallReturnPercent =
		totalInv > 0 ? ((totalVal - totalInv) / totalInv) * 100 : 0;
	const dayChange = totalVal - prevDayTotalVal;
	const dayChangePercent =
		prevDayTotalVal > 0 ? (dayChange / prevDayTotalVal) * 100 : 0;
	...
	const healthScore = Math.round(
		returnScore * 0.4 + diversityScore * 0.2 + wrScore * 0.2 + pfScore * 0.2,
	);
	...
	const activeDays = Math.ceil(
		(Date.now() - firstTradeDateVal) / (1000 * 60 * 60 * 24),
	);
	const avgTradesPerDay =
		tradingDays > 0 ? transactions.length / tradingDays : 0;

	// Win rate for CURRENT holdings only (ACCURATE)
	const holdingsWinRate =
		holdings.length > 0 ? (winnersCount / holdings.length) * 100 : 0;
	const totalStocksTraded = txnMap.size;
	const currentlyHolding = holdings.length;

	// Sorting
	const sortedByPerf = [...holdings].sort(
		(a, b) => b.percentProfit - a.percentProfit,
	);
	const sortedByVal = [...holdings].sort(
	...
	const insights: AdvancedUserInsights = {
		...
		trading: {
			...
			mostActiveScripCount: mostActive
				? mostActive.buyCount + mostActive.sellCount
				: 0,
